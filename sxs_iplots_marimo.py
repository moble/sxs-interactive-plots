...
"""
def iplt_lm(h, t, metadata, Mass, Distance, cut=False):

    f_x_SPA_test, f_y_SPA_test, f_x_strain_test, f_y_strain_test = create_functions(h, t, metadata, cut)

    plt.plot(f_x_SPA_test(Mass, Distance), f_y_SPA_test(Mass, Distance), color="pink")
    plt.plot(f_x_strain_test(Mass, Distance), f_y_strain_test(Mass, Distance), color="lightblue", ls="--")
    plt.xlim(3, 5e3)
